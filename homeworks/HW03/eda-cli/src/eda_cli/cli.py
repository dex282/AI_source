...
def report(
    ...
    out_dir: Optional[Path] = typer.Option(
        None, "--out-dir", "-o", help="Директория для сохранения отчёта"
    ),
    sep: str = ",",
    encoding: str = "utf-8",
    max_hist_columns: int = typer.Option(
        5,
        "--max-hist-columns",
        help="Максимальное число числовых колонок для гистограмм",
    ),
    top_k_categories: int = typer.Option(
        10,
        "--top-k-categories",
        help="Сколько top-значений показывать для категориальных колонок",
    ),
    title: str = typer.Option(
        "EDA Report",
        "--title",
        help="Заголовок отчёта (первая строка в report.md)",
    ),
    min_missing_share: float = typer.Option(
        0.1,
        "--min-missing-share",
        help="Порог доли пропусков для проблемных колонок (0.0–1.0)",
    ),
) -> None:
    """
    ...
    """
    # 0. Подготовка выходной директории
    out_root = Path(out_dir) if out_dir is not None else Path("reports")
    out_root.mkdir(parents=True, exist_ok=True)

    # 1. Чтение данных
    df = _load_csv(Path(path), sep=sep, encoding=encoding)

    # 2. Обзор и базовые таблицы
    ...
    # 4. Сохранение табличных артефактов
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Макс. число колонок для гистограмм: **{max_hist_columns}**\n")
        f.write(f"- Top-K категорий: **{top_k_categories}**\n")
        f.write(f"- Порог проблемных колонок по пропускам: **{min_missing_share:.1%}**\n\n")
